...
function Set-JavaOpts {
    if (Test-Path $PWD\_genonce.sh) {
        ((Get-Content -path $PWD\_genonce.sh -Raw) -replace 'java -jar','java $JAVA_OPTS -jar') | Set-Content -Path $PWD\_genonce.sh
    }
}
